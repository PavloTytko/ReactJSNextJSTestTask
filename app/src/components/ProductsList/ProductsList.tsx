        ...
        {products.map((p) => (
          <div key={`card-${p.id}`} className={styles.cardItem}>
            ...
            <div className={styles.cardBody}>
              ...
              <div>
                <span className={styles.label}>{t('products.table.type')}:</span> {p.type}
              </div>
              <div>
                <span className={styles.label}>{t('products.table.serial')}:</span>{' '}
                {p.serialNumber || t('common.na')}
              </div>
              <div>
                <span className={styles.label}>{t('products.table.guarantee')}:</span>{' '}
                {p.guarantee ? `${p.guarantee.start} → ${p.guarantee.end}` : t('common.na')}
              </div>
              <div>
                <span className={styles.label}>{t('products.table.price')}:</span>{' '}
                {p.price?.map((pr) => `${pr.value} ${pr.symbol}`).join(', ')}
              </div>
              <div>
                <span className={styles.label}>{t('products.table.order')}:</span>{' '}
                {p.order ?? t('common.na')}
              </div>
            </div>
          </div>
