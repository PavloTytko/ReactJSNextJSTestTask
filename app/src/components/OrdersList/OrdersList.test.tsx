...
jest.mock("react-i18next", () => ({
  useTranslation: () => ({ t: (k: string, opts?: any) => {
    if (k === "orders.deleteAria" && opts?.title) return `Delete ${opts.title}`;
    return k;
  } }),
}));

const orders: Order[] = [
  { id: 1, title: "Order A", date: new Date().toISOString(), description: "", products: [] },
  { id: 2, title: "Order B", date: new Date().toISOString(), description: "", products: [] },
];

describe("OrdersList", () => {
  it("shows loading text when loading is true", () => {
    render(
      <OrdersList orders={[]} loading onSelect={jest.fn()} onDelete={jest.fn()} />
    );
